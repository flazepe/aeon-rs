...
impl ArgGetters for CommandInput {
    fn get_string_arg<T: ToString>(&self, arg: T) -> Result<String> {
        Ok(self.args.get(&arg.to_string()).context("Could not get arg.")?.as_string().context("Could not convert arg to String.")?)
    }

    fn get_i64_arg<T: ToString>(&self, arg: T) -> Result<i64> {
        Ok(self.args.get(&arg.to_string()).context("Could not get arg.")?.as_i64().context("Could not convert arg to i64.")?)
    }

    fn get_bool_arg<T: ToString>(&self, arg: T) -> Result<bool> {
        Ok(self.args.get(&arg.to_string()).context("Could not get arg.")?.as_bool().context("Could not convert arg to bool.")?)
    }

    fn get_user_arg<T: ToString>(&self, arg: T) -> Result<&User> {
        Ok(self.args.get(&arg.to_string()).context("Could not get arg.")?.as_user().context("Could not convert arg to User.")?)
    }

    fn get_channel_arg<T: ToString>(&self, arg: T) -> Result<&Channel> {
        Ok(self.args.get(&arg.to_string()).context("Could not get arg.")?.as_channel().context("Could not convert arg to Channel.")?)
    }

    fn get_role_arg<T: ToString>(&self, arg: T) -> Result<&Role> {
        Ok(self.args.get(&arg.to_string()).context("Could not get arg.")?.as_role().context("Could not convert arg to Role.")?)
    }

    fn get_f64_arg<T: ToString>(&self, arg: T) -> Result<f64> {
        Ok(self.args.get(&arg.to_string()).context("Could not get arg.")?.as_f64().context("Could not convert arg to f64.")?)
    }
...
impl AvatarURL for User {
    ...
    fn display_avatar_url<T: Display, U: Display>(&self, format: T, size: U) -> String {
        match self.avatar_url(format, size) {
            ...
            None => format!("https://cdn.discordapp.com/embed/avatars/{}.png", self.discriminator.parse::<u64>().unwrap() % 5),
        }
    }
}
